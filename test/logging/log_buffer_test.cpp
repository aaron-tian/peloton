...
TEST_F(LogBufferTests, InsertLogBufferTest) {
  ...
  tile_column_names.emplace_back("INTEGER COL");
  ...
  std::shared_ptr<storage::TileGroup> tile_group(storage::TileGroupFactory::GetTileGroup(
    database_id, table_id, tile_group_id, nullptr, schemas, layout, 3));
  storage::StorageManager::GetInstance()->AddTileGroup(block, tile_group);

  logging::LogRecord insert_record = logging::LogRecordFactory::CreateTupleRecord(
    LogRecordType::TUPLE_INSERT, location, epoch_id, txn_id, commit_id, schema_id);
  ...
  size_t buffer_size = sizeof(int32_t) + sizeof(int8_t) + sizeof(int64_t) * 3 + sizeof(int64_t) * 5 +
                        sizeof(double) * 3;
  ...
  EXPECT_EQ(schema_id, log_buffer_input.ReadLong());
...
TEST_F(LogBufferTests, UpdateLogBufferTest) {
  ...
  size_t buffer_size = sizeof(int32_t) + sizeof(int8_t) + sizeof(int64_t) * 3 + sizeof(int64_t) * 8 +
...
TEST_F(LogBufferTests, DeleteLogBufferTest) {
  ...
  logging::LogRecord delete_record = logging::LogRecordFactory::CreateTupleRecord(
                                       LogRecordType::TUPLE_DELETE, old_location, epoch_id, txn_id, commit_id, schema_id);
